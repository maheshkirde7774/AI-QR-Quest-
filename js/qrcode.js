 ...
 * Supports rendering directly to HTML5 Canvas and SVG.
 */
(function (global) {
  'use strict';

  // QR Code Model Implementation (Byte mode, Error Correction Levels: L, M, Q, H)
  ...
  QRCode.prototype.makeCode = function (text) {
    ...
    var canvas = (this._target && this._target.tagName === "CANVAS") ? this._target : document.createElement("canvas");
    var ctx = canvas.getContext("2d");
    var count = qrcode.getModuleCount();
    var quiet = this.options.quietZone || 2;
    var totalCells = count + quiet * 2;
    var size = Math.min(this.options.width, this.options.height);
    canvas.width = size;
    canvas.height = size;
    ...
    for (var row = 0; row < count; row++) {
      for (var col = 0; col < count; col++) {
        if (qrcode.isDark(row, col)) {
          var x = Math.round((col + quiet) * cellSize);
          var y = Math.round((row + quiet) * cellSize);
          var w = Math.ceil((col + quiet + 1) * cellSize) - x;
          var h = Math.ceil((row + quiet + 1) * cellSize) - y;
          ctx.fillRect(x, y, w, h);
        }
      }
    }

    if (this._target && this._target !== canvas) {
  ...
  QRCode.toCanvas = function (targetCanvas, text, options) {
    ...
    var qr = new QRCode(targetCanvas, opts);
    return targetCanvas;
  };
